We're continuing our implementation of Remaining Lecture Session Capture Flow Features by implementing task group number 2:
...
- [ ] 2.0 Complete real capture runtime integration
  - [ ] 2.1 Write 2-8 focused tests for capture runtime orchestration
  ...
  - [ ] 2.2 Implement capture runtime adapter boundary
  ...
  - [ ] 2.3 Wire adapter into `capture start` / `capture stop`
  ...
  - [ ] 2.4 Preserve command output/error contracts
  ...
  - [ ] 2.5 Ensure Task Group 2 tests pass
...
Read @agent-os/specs/2026-03-06-remaining-lecture-session-capture-flow-features/spec.md to understand the context for this spec and where the current task fits into it.
...
For example, if you've been assigned implement the 3rd task from `tasks.md` and that task's title is "Commenting System", then you must create the file: `agent-os/specs/2026-03-06-remaining-lecture-session-capture-flow-features/implementation/3-commenting-system-implementation.md`.
...
## Overview
**Task Reference:** Task #[number] from `agent-os/specs/2026-03-06-remaining-lecture-session-capture-flow-features/tasks.md`
