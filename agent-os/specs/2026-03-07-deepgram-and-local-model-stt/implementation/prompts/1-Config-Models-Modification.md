We're continuing our implementation of Deepgram and Local Model STT by implementing task group number 1:
...
- [ ] 1.0 Complete configuration layer updates
  - [ ] 1.1 Write 2-8 focused tests for `STTConfig` and `STTResult` changes
    - Test validation logic for new mode properties (e.g. language, diarization, deepgram provider)
  - [ ] 1.2 Update `STTConfig` in `src/lecture_auto/stt_config.py`
    - Add `language` field
    - Add `diarization` bool field
    - Expand provider validations (e.g. deepgram API)
  - [ ] 1.3 Update `STTResult` in `src/lecture_auto/stt_runtime.py`
    - Add fields to support storing speaker diarization mapping timestamps
  - [ ] 1.4 Ensure config layer tests pass
...
Read @agent-os/specs/2026-03-07-deepgram-and-local-model-stt/spec.md to understand the context for this spec and where the current task fits into it.
...
For example, if you've been assigned implement the 3rd task from `tasks.md` and that task's title is "Commenting System", then you must create the file: `agent-os/specs/2026-03-07-deepgram-and-local-model-stt/implementation/3-commenting-system-implementation.md`.
...
## Overview
**Task Reference:** Task #[number] from `agent-os/specs/2026-03-07-deepgram-and-local-model-stt/tasks.md`
